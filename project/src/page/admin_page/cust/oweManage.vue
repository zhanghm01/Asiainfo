<template>
  <div id='ordersearch-container'>
    <div class="content">
      ...
      <hr>
      <!-- 添加 批量删除 下载Excel模板 上传Excel文件 按钮 -->
      <div class="button" style='display:flex;'>
        <el-button type="primary" icon="el-icon-edit" style='font-size:8px;' @click='beginAdd'>添加</el-button>
        <el-button type="primary" icon="el-icon-edit" style='font-size:8px;' @click='PLhandleDelete'>批量删除</el-button>
        <a icon="el-icon-edit" id='downloadtemplate' :href='downloadhttp' download=""  title="下载" >download</a>
        <!-- <el-upload class="upload-demo" action="#" :on-preview="handlePreview" :on-remove="handleRemove"  :before-remove="beforeRemove" multiple :limit="1" :on-exceed="handleExceed" :file-list="fileList" style='margin:0 20px;'> -->
          <el-button type="file" icon="el-icon-edit" style='font-size:8px;'>upload</el-button>
        <!-- </el-upload> -->
      </div>
      <hr>
      <!-- 数据表格区域 -->
      ...
      <div class="operators" v-if='isopen'>
        ...
        <div class="deletebox" v-if='isdelete'>
           <div class="content">
              您将删除以下数据：接入机房地址ID为{{this.deleteObj}}
              确认删除？
            </div>
          <div class="btns">
...
export default {
    data() {
      return {
        // fileList: [],
        downloadhttp:'http://10.21.19.86:8080/sdc/excel/download?moduleName=Address',
        target:{
          currentPage:1,
          pageNum: 1,
          pageSize: 3,
        ...
        //最好是全国各省市区名称：新增数据时并不能确定要添加的是哪个省市区
        ChineseDistricts:[
          {
              "code":"01",
              "name":"北京市",
  ...
  methods: {
    ...
    getAllData(obj){
      obj = Object.assign({ pageSize:"", pageNum:""},this.orderObj);
      getJieDianData(obj).then(res=>{
          // console.log('登录响应成功')
          console.log('看看获取到了什么数据')
          console.log(res)
          ...
          // console.log(result)
          if(result.retCode == '200'){
            this.Chinese = result.data[0].list;
          }
          console.log('获取全部数据')
    ...
    cancelAdd(){
      ...
      //点击取消时，直接将数据置为空
      Object.keys(this.newObj).map(key => this.newObj[key] = "")
      //由于新增数据时的可选数据与条件查询数据绑定，因此也需要将条件查询数据置为空
      this.orderObj = {province:"",city:"",district:"",dc_address:""};
    },
    // 点击修改按钮，开遮罩
    ...
    PLhandleDelete(){
      this.multipleSelection.forEach((item)=>{
        this.deleteObj.push(
          {dc_addr_id:item.dc_ADDR_ID}
        )
      })
      return ((obj)=>{
        obj = Object.assign(JSON.parse(JSON.stringify(this.deleteObj)));
        console.log('删除的数据ID:',obj)
        deleteJieDianData(obj)
        .then(res=>{
          console.log('删除数据响应成功');
          console.log(res);
          console.log(obj);
